...
export default function PacienteVinculado({id, nome, idade, cidade, handleClick}) {
  return (
    <div className="w-[90%] h-full shadow-2xl rounded-xl mx-auto hover:scale-110 hover:cursor-pointer duration-500 ease-in-out" onClick={() => handleClick({id, nome, idade, cidade})}>
      ...
      <div className="w-full h-[45%] py-2 px-5">
        <h2 className={`font-nunito ${nome.length > 10 ? "text-2xl" : "text-3xl"}`}>{nome}</h2>
        <p className="font-nunito text-xl">{idade}</p>
        <p className="font-nunito text-xl">{cidade}</p>
      </div>
    </div>
  )
}